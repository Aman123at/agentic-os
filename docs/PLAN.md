...
| `SystemService` | `Info` (Mode, versions, Landlock, Host hints), `Processes`, `Audit`; `Usage` (M4: model usage per day, for the Agent app's chart), `Metrics` (M4: the Machine's CPU, memory, disks and network, read from its own cgroup first; polled by Activity Monitor while open) |
