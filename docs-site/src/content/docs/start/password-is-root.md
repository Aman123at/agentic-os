...
[Audit Log](/reference/audit/). The asking is real: nothing privileged happens without
...
3. **[Put TLS in front](/configure/nginx/)** if the server holds anything real. Plain
...
4. **Know the recovery path before you need it.** There is no email reset, because there is no
   email. [`sudo aos user passwd` over SSH](/operate/lost-password/) is the only way
...
Run it [with Docker Compose](/start/compose/) instead. Agents get a full Ubuntu machine
