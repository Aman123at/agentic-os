...
- **Chromium's own sandbox on a native install.** `--no-sandbox` was justified by Docker's seccomp profile and excused by the container; natively neither exists. Owed to *Installing Chromium lazily* (12), and an M6 code sub-task either way, alongside `internal/agent/instructions.go:38` telling every Agent "There is no systemd".
...
- **What `aos doctor` means without a container.** Half its checks (`--host-check`, forwarding, the secret mount) describe a Host/Machine boundary that no longer exists natively. The replacement check-list depends on the sandbox and systemd decisions, **both of which are now settled, so this is unblocked**.
