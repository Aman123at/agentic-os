...
## Input from *The glossary after Machine and Host collapse* (resolved 2026-09-17)

`CONTEXT.md` is rewritten in that ticket's own commit, so M6 inherits the vocabulary rather than deciding it: **Host is retired** (the computer AOS runs on is the Machine; the computer the user browses from is *your computer*, lowercase), the two shapes are the fixed phrases **native install** and **Compose install**, and **Daemon** (`aosd`, one systemd unit) is now glossary-distinct from **Service** (a program the user asked AOS to keep running, which is *not* a systemd unit).

The PLAN sweep M6 must carry:

- **§15 is replaced, not amended.** "Cross-Host support" retires with the word. Five of its nine rows describe Docker Desktop on a laptop (Runtime, Landlock, the two Shared Folder rows, Start command) and die or move to §6; three are client-side and survive (Service subdomains, Reserved shortcuts, Verified by). The new section is a two-row table for the two install shapes plus a short browser/OS table.
- **§12 gets five corrections.** (a) The two forwarding forms swap primacy: `/port/<n>/` is canonical, `<port>.localhost` a local convenience that cannot resolve from a remote browser. (b) The cookie-stripping bullet describes a cookie that no longer exists; ticket 03's ticket-exchanged cookie scoped to `Path=/port/<n>/` replaces it. (c) The `compose.override.yaml` bullet is marked Compose-only. (d) The low-ports bullet is a **container fact stated as a product fact** — M0.7 measured `ip_unprivileged_port_start=0` inside Docker, which sets it, while the kernel default is 1024 — so it is rewritten as a difference between the two shapes, pending one command on the VPS. (e) See the next sub-task.
- **§1, §2 and §22's Hosts row** still frame the product as `docker compose up` on macOS, Windows and Linux Hosts. §2's "Services with port forwarding to the Host" and "Host-side CLI" both use the retired word.

Three code sub-tasks the sweep found:

- **A wildcard listener is a publish, and AOS says nothing.** Under Docker an unpublished port is unreachable — the network namespace is the barrier. Natively there is none, so an Agent that starts a dev server on `0.0.0.0:3000` has put it on the internet, bypassing the forwarder, the Account and the whole of tickets 01/02/03. Activity Monitor, the Services list and `aos status` flag a listener on `0.0.0.0` or `::` as reachable from outside — `internal/service/ports.go:144` already has the `wildcard()` helper — and the Agent prompt gains one sentence telling Agents to bind Services to `127.0.0.1` unless the user asked for public. Documenting it alone is not enough: the product tells Agents to start servers, and natively that is a publish.
- **The Machine Profile, three lines.** `internal/profile/profile.go:86-88` repeats the falsehoods booked against the Agent prompt, in a file nobody checked: `~/Shared` (already booked via ticket 07), `http://<port>.localhost:<AOS port>` as the way to reach a Service, and "outside the home folder, only software from install_package and /etc changes by root survive a restart". It is rewritten alongside the prompt's Machine paragraph, not after it — the two texts are read by the same Agent in the same request.
