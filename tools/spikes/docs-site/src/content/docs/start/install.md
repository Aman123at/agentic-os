...
- **systemd.** The installer checks for it and stops cleanly if it is missing, because there is no
  second lifecycle to fall back on. [Docker Compose](/agentic-os/start/compose/) is the supported
  alternative.
- **root**, through `sudo`.
...
- An **OpenAI API key**. You do not need it to install — the daemon starts without one and
  [you add it afterwards](/agentic-os/configure/config-file/).
...
[Install Ledger](/agentic-os/reference/software/), and you can undo it.
...
- **It is reachable from the internet.** The daemon binds `0.0.0.0`, so a fresh install answers on
  ...
  [what the password protects](/agentic-os/start/password-is-root/).
...
- **It is plain HTTP.** Your password crosses the network in the clear. If that is not acceptable
  ...
  [put nginx and a certificate in front](/agentic-os/configure/nginx/) before you sign in the
  first time.

## Next

1. [Sign in and set your own password](/agentic-os/start/sign-in/)
2. [Add your API key and choose a model](/agentic-os/configure/config-file/)
3. [Give it something to do](/agentic-os/reference/run/)
